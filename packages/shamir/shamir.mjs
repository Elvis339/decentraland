      ...
      for (let i = 0; i < x.length; i++) {
        if (y[i]) {
          ...
          for (let j = 0; j < x.length; j++) {
            if (i !== j) {
              product =
                (product + calculatedLogarithms[0 ^ x[j]] - calculatedLogarithms[x[i] ^ x[j]] + maxShares) % maxShares;
            }
          }
...
export let Shamir = {
  ...
  generateShares: function (secret, totalShares, requiredShares, padLength) {
    ...
    if (totalShares > maxShares) {
      ...
      throw new Error(
        `Number of shares must be an integer between 2 and 2^bits-1 (${maxShares}), inclusive. To create ${totalShares} shares, use at least ${neededBits} bits.`,
      );
    }

    if (typeof requiredShares !== 'number' || requiredShares % 1 !== 0 || requiredShares < 2) {
      throw new Error(
        `Threshold number of shares must be an integer between 2 and 2^bits-1 (${maxShares}), inclusive.`,
      );
    }
    ...
    if (requiredShares > maxShares) {
      ...
      throw new Error(
        `Threshold number of shares must be an integer between 2 and 2^bits-1 (${maxShares}), inclusive.  To use a threshold of ${requiredShares}, use at least ${neededBits} bits.`,
      );
    }

    if (requiredShares > totalShares) {
      throw new Error(
        `Threshold number of shares was ${requiredShares} but must be less than or equal to the ${totalShares} shares specified as the total to generate.`,
      );
    }
    ...
    for (let i = 0; i < totalShares; i++) {
      ...
      if (
        typeof integerShareId !== 'number' ||
        integerShareId % 1 !== 0 ||
        integerShareId < 1 ||
        integerShareId > maxShares
      ) {
